...
import static com.example.user.myapplication.Constants.ARENA_NONE;
...
import static com.example.user.myapplication.Cells.Type2.EMPTY;
import static  com.example.user.myapplication.Cells.Type2.OBSTACLE;
...
public class ArenaView extends SurfaceView implements SurfaceHolder.Callback {
   ...
   // private MainThread thread;
     ...
     */
    private Cells getTouchedCell(float x, float y) {
    ...
    public String generatePart1MapDescriptor() {
        ...
        for (int i = ARENA_ROW_COUNT - 1; i > -1; i--) {
            for (int j = 0; j < ARENA_COLUMN_COUNT; j++) {
                ...
                if (cells[i][j].getType().equals(Cells.Type.UNEXPLORED)
                        || cells[i][j].getType().equals(Cells.Type.UNKNOWN)) {
                    sb.append("0");
                } else {
                    sb.append("1");
                }
    ...
    public String generatePart2MapDescriptor() {
        ...
        for (int i = ARENA_ROW_COUNT - 1; i > -1; i--) {
            for (int j = 0; j < ARENA_COLUMN_COUNT; j++) {
                //  Set value 0 if unexplored/unknown, 1 if otherwise
                if (cells[i][j].getType().equals(Cells.Type.EXPLORED)) {
        ...
        for (int i = ARENA_ROW_COUNT - 1; i > - 1; i--) {
            for (int j = 0; j < ARENA_COLUMN_COUNT; j++) {
                ...
                if (Integer.parseInt(character) == 0) {
                    cells[i][j].setType(Cells.Type.UNEXPLORED);
                } else {
                    cells[i][j].setType(Cells.Type.EXPLORED);
                }
                k++;
            }
        }
    ...
    public void updateMapP2(String paddedMapDescriptorP2) {
        ...
        for (int i = ARENA_ROW_COUNT - 1; i > -1; i--) {
            for (int j = 0; j < ARENA_COLUMN_COUNT; j++) {
                if (cells[i][j].getType() == Cells.Type.EXPLORED) {
                    //  Get each binary digit
                    character = String.valueOf(sb.charAt(k));
                    //  Assign cell type based on binary value
                    if (Integer.parseInt(character) == 0) {
  ...
  //  private void sendRobotStartingPosition() {
   ...
   // }
    ...
    public void setWaypoint(int x, int y) {
        ...
        if (wp != null) {
            wp.setType3(Cells.Type3.NONE);
        }

        //  Add new way point
        wp = cells[ARENA_ROW_COUNT - y - 1][x];
        wp.setType3(Cells.Type3.WAYPOINT);
    }
     ...
     */
    public void setFragment(main_fragment parent) {
        mFragment = parent;
    }
